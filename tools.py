...
from storage_supabase import add_claim_record
...
async def generate_claim_id(call_sid: str) -> str:
    """Generate a unique claim ID (format: 2-3 letters, dash, 10 digits with 000)."""
...
async def store_answer(call_sid: str, question: str, answer: str) -> str:
    """Store an answer to a claim question in Supabase."""
    claim_id = get_claim_id(call_sid)
    if not claim_id:
        logger.warning(f"No claim ID found for call {call_sid}")
        return "No claim ID found. Please generate a claim ID first."

    success = await add_claim_record(call_sid, claim_id, question, answer)
    return (
        f"Stored answer for claim {claim_id}" if success else "Failed to store answer"
    )
